@php
    $isSyncRunning = (bool) ($syncStatus['is_running'] ?? false);
    $sidebarIcons = [
...
<aside data-sidebar class="fixed inset-y-0 left-0 z-30 w-72 -translate-x-56 border-r border-slate-200 bg-white shadow-xl transition-all duration-200 dark:border-slate-800 dark:bg-slate-950 xl:sticky xl:top-0 xl:h-screen xl:w-auto xl:translate-x-0 xl:shadow-none">
    <div class="flex h-full flex-col gap-4 p-4 xl:gap-6">
        <div class="flex items-center justify-between gap-3">
            ...
            <button type="button" data-sidebar-toggle aria-label="Toggle sidebar" aria-expanded="true" title="Toggle sidebar" class="ml-auto grid h-10 w-10 shrink-0 place-items-center rounded-md border border-slate-200 bg-white text-slate-600 shadow-sm hover:border-teal-300 hover:text-teal-800 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300 dark:hover:border-teal-500 dark:hover:text-teal-300">
        ...
        <div data-mobile-sidebar-panel class="hidden flex-1 flex-col gap-4 xl:flex xl:gap-6">
            ...
            <div class="mt-auto grid gap-2">
                <form method="POST" action="{{ route('ghl.sync') }}">
                    @csrf
                    <button type="submit" title="{{ $isSyncRunning ? 'HighLevel sync is already running.' : 'Run one full GHL sync to the database. Automatic batches are handled in the background.' }}" @disabled($isSyncRunning) class="flex w-full items-center justify-center gap-2 rounded-md px-3 py-2 text-sm font-semibold shadow-sm {{ $isSyncRunning ? 'cursor-not-allowed bg-slate-200 text-slate-500 dark:bg-slate-800 dark:text-slate-400' : 'bg-slate-950 text-white hover:bg-slate-800 dark:bg-white dark:text-slate-950 dark:hover:bg-slate-200' }}">
                        <svg viewBox="0 0 24 24" class="h-4 w-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                        ...
                        <span data-sidebar-label>{{ $isSyncRunning ? 'Sync in progress' : 'Sync GHL data' }}</span>
                    </button>
                </form>
                <p data-sidebar-content class="px-1 text-center text-[11px] leading-4 text-slate-500 dark:text-slate-400">
                    {{ $isSyncRunning ? 'Wait until the current sync finishes before starting another one.' : 'One click runs the full sync. Large tags are split automatically into background batches.' }}
                </p>
